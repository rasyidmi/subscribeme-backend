...
type EventRepository interface {
	Create(event models.Event, classsesID []int) error
...
func (r *eventRepository) Create(event models.Event, classsesID []int) error {
...
func (r *eventRepository) FindByID(id int) (models.EventResponse, error) {
	var event models.EventResponse
	err := r.DB.Debug().Model(&models.Event{}).Find(&event, id).Error
	if err != nil {
...
func (r *eventRepository) UpdateByID(id int, newData map[string]interface{}) error {
	...
	err = r.DB.Model(&models.Event{}).Where("id = ?", id).Updates(newData).Error
	return err
}
